...
## Про автодобычу данных и блокировки

**realestate.com.au сидит за Akamai Bot Manager** и блокирует автоматический
доступ (HTTP 429 / Access Denied) — проверено: не проходят ни curl, ни Jina,
ни headless/headed Chromium с домашнего IP. Бесплатно и надёжно вытащить
адрес/цену/комнаты с него нельзя.

Поэтому рабочий процесс такой:

1. Из URL бот всегда достаёт **тип и район** + ссылку — это бесплатно и всегда.
2. Если рядом со ссылкой написать адрес/цену — бот разложит их по полям.
3. Любые детали дописываются боту словами:
   «#1 — 2 спальни, 1 санузел, $750 в неделю, инспекция в субботу 11:00».

**Полная автодобыча (опционально, платно):** заведи ключ на
[zenrows.com](https://www.zenrows.com) или [scraperapi.com](https://scraperapi.com)
с резидентными прокси и впиши `ZENROWS_API_KEY` в переменные — код переключится
на него, когда обычный запрос не проходит. На бесплатном триале хватает на
несколько десятков квартир.

domain.com.au защищён слабее — с него детали могут подтягиваться сами.
